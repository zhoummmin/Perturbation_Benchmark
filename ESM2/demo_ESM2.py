...
pert_data = PertData('./All_data', default_pert_graph=False) 
pert_data.load(data_path = f'./All_data/{data_name_lower}')
...
for seed in range(1, 6):
    ...
    gears_model = GEARS(pert_data, device=device,
                        ...
                        proj_name='New_gears',
                        exp_name=f'gears_esm_{data_name}_split{seed}')
    gears_model.model_initialize(hidden_size=64)
    gears_model.train(epochs=15, lr=1e-3)

    model_save_path = f'/home/share/huadjyin/home/zhoumin3/zhoumin/model_benchmark/01_A_results/{data_name}/gears_esm/split{seed}'
    ...
    results_to_save = [
        (test_result, f"{data_name}_split{seed}_test_res"),
        (compute_metrics(test_result), f"{data_name}_split{seed}_test_metrics"),
        (test_result, f"{data_name}_split{seed}_test_pert_res"),
        (deeper_analysis(pert_data.adata, test_result), f"{data_name}_split{seed}_deeper_res"),
        (non_dropout_analysis(pert_data.adata, test_result), f"{data_name}_split{seed}_non_dropout_res"),
        (non_zero_analysis(pert_data.adata, test_result), f"{data_name}_split{seed}_non_zero_res")
    ]

    def save_results(results, file_prefix, save_path):
