...
pert_data.load(data_path = './All_data/adamsonweissman2016_gsm2406675_1')
...
for seed in range(1, 6):
    ...
    with open("../scbert_gene_embedding_16906.pkl", "rb") as fp:
        GPT35_gene_embeddings = pickle.load(fp)
    gene_names= list(pert_data.adata.var['ensembl_id'].values)
    count_missing = 0
    EMBED_DIM = 200 # embedding dim from GPT-3.5
    lookup_embed = np.zeros(shape=(len(gene_names),EMBED_DIM))
    for i, gene in enumerate(gene_names):
        if gene in GPT35_gene_embeddings:
            lookup_embed[i,:] = GPT35_gene_embeddings[gene].flatten()
        else:
            count_missing+=1
    ...
    gears_model = GEARS(pert_data, device = device, 
                            ...
                            proj_name = 'New_gears', 
                            exp_name = f'scbert_{data_name}_split{seed}')
    gears_model.model_initialize(hidden_size = 64)
    
    gears_model.train(epochs = 15, lr = 1e-3)

    model_save_path = f'/home/share/huadjyin/home/zhoumin3/zhoumin/model_benchmark/01_A_results/{data_name}/new_scbert/split{seed}'
