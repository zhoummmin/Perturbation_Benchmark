...
def loss_fct(pred, y, perts, ctrl = None, direction_lambda = 1e-3, dict_filter = None):
    ...
    for p in set(perts):
        ...
        if p!= 'ctrl':
            ...
            #try:  # zhoumin
                #retain_idx = dict_filter[p]
            #except KeyError:  # zhoumin
                #print(f"Key '{p}' not found in dict_filter. Skipping this key...")
                #continue
        ...
        losses = losses + torch.sum((pred_p - y_p)**(2 + gamma)) / (pred_p.shape[0] * pred_p.shape[1])  #zhoumin
