...
def get_go_auto(gene_list, data_path, data_name):
    ...
    if os.path.exists(go_path):
        ...
        with open(os.path.join('/home/share/huadjyin/home/zhoumin3/AttentionPert/gene2go_all.pkl'), 'rb') as f:
...
def loss_fct(pred, y, perts, ctrl = None, direction_lambda = 1e-3, dict_filter = None):
    ...
    for p in set(perts):
        ...
        pert_idx = torch.tensor(pert_idx, dtype=torch.long) if isinstance(pert_idx, np.ndarray) else pert_idx #zhoumin
        ...
        if p!= 'ctrl':
            ...
            #print("dict_filter_shape", dict_filter.shape)
            retain_idx = torch.tensor(retain_idx, dtype=torch.long) if isinstance(retain_idx, np.ndarray) else retain_idx # zhoumin
            pred_p = pred[pert_idx][:, retain_idx]
            print("y shape:", y.shape)
            print("pert_idx:", pert_idx)
            print("retain_idx:", retain_idx)
