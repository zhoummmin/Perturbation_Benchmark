    ...
    for itr, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for itr, de_idx in enumerate(batch.de_idx):
                de_idx = torch.tensor(de_idx, dtype=torch.long) if isinstance(de_idx, np.ndarray) else de_idx # zhoumin
                current_pred_de = p[itr, de_idx]
                current_truth_de = t[itr, de_idx]
                if len(current_pred_de) > 19:
                    current_pred_de = current_pred_de[:19]
                    current_truth_de = current_truth_de[:19]
                pred_de.append(current_pred_de)
                truth_de.append(current_truth_de)
...
def evaluate_old(loader, model, uncertainty, device, real_num_genes = None):
    """
    Run model in inference mode using a given data loader
    """
    ...
    # all genes
    for i, tensor in enumerate(pred_de):
        print(f"pred_de中第{i}个张量的形状: {tensor.shape}")
...
def non_zero_analysis(adata, test_res):
    ...
    for pert in np.unique(test_res['pert_cat']):
        ...
        nonzero_des = np.setdiff1d(list(range(19)), zero_des)
