...
from .model import *
from .inference import evaluate, compute_metrics, deeper_analysis, non_zero_analysis, \
...
class ATTNPERT_RECORD_TRAIN:
    def __init__(self, pert_data, 
        ...
        #self.ctrl_expression = torch.tensor(np.array(self.adata.X[self.adata.obs.condition == 'ctrl'].mean(axis=0)).flatten(), dtype=torch.float32).to(self.device)  # zhoumin
    ...
    def train(self, epochs = 20, 
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                if self.config['uncertainty']:
                    ...
                    print("self.real_num_genes:", self.real_num_genes)
        ...
        if save_path:
            ...
            with open(os.path.join(save_path, f'{data_name}_split{seed}_test_res.pkl'), 'wb') as f:
                pickle.dump(test_res, f)     
            with open(os.path.join(save_path, f'{data_name}_split{seed}_test_metrics.pkl'), 'wb') as f:
                pickle.dump(test_metrics, f)       
            with open(os.path.join(save_path, f'{data_name}_split{seed}_test_pert_res.pkl'), 'wb') as f:
                pickle.dump(test_pert_res, f)
        
            print_sys(f"Results saved to {save_path}")
        ...
        non_zero_res = non_zero_analysis(self.adata, test_res)
        
        
        if save_path:
            os.makedirs(save_path, exist_ok=True)
            with open(os.path.join(save_path, f'{data_name}_split{seed}_deeper_res.pkl'), 'wb') as f:
                pickle.dump(out, f)     
            with open(os.path.join(save_path, f'{data_name}_split{seed}_non_dropout_res.pkl'), 'wb') as f:
                pickle.dump(out_non_dropout, f) 
            with open(os.path.join(save_path, f'{data_name}_split{seed}_non_zero_res.pkl'), 'wb') as f:
