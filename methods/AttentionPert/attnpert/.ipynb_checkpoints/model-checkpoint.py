...
class PL_PW_non_add_Model(torch.nn.Module):
    ...
    def __init__(self, args: dict):
        ...
        if self.ablation_strings["g2v"] == "Y":
            ...
            gene2vec_weight = torch.tensor(gene2vec_weight, dtype=torch.float32)  # 使用 torch.tensor zhoumin
    ...
    def forward(self, data, is_train = False):
        ...
        if self.ablation_strings["pw"] != "N":
            ...
            print("torch.LongTensor(list(range(self.num_genes))).shape:", torch.LongTensor(list(range(self.num_genes))).shape)
            print("self.pert_weight_emb expected input shape:", self.pert_weight_emb.weight.shape[0])  # 假设可以通过查看嵌入层权重的第一个维度来大致了解期望输入的维度情况
