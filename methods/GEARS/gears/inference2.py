...
def deeper_analysis(adata, test_res, de_column_prefix = 'rank_genes_groups_cov', most_variable_genes = None):
    ...
    for pert in np.unique(test_res['pert_cat']):
        ...
        for val in [20, 50, 100, 200]:
            ...
            mae = np.mean(np.abs(test_res['pred_de'][pert_idx] - test_res['truth_de'][pert_idx]))
            pert_metric[pert]['MAE'] = mae

       # for val in [20, 50, 100, 200]:
            # MAE
        #    mae_value = np.mean(np.abs(pred_mean[de_idx_map[val]] - true_mean[de_idx_map[val]]))
         #   pert_metric[pert]['mae_top_' + str(val)] = mae_value
