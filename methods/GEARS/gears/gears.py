...
class GEARS:
    ...
    def model_initialize(self, hidden_size = 64,
                         ...
                         no_perturb = False,
                         #no_perturb = True,
                         cell_fitness_pred = False, # zhoumin
                        ):
        """
        ...
        self.config = {'hidden_size': hidden_size,
                       ...
                       'cell_fitness_pred': cell_fitness_pred,    # zhoumin
                      }
    ...
    def plot_perturbation(self, query, save_file = None):
        ...
        pred_results = self.predict([query_]) # zhoumin
        if isinstance(pred_results, tuple): # zhoumin
            pred_results = pred_results[0]  #zhoumin
        pred = pred_results['_'.join(query_)][de_idx] # zhoumin
       ...
       # pred = self.predict([query_])['_'.join(query_)][de_idx]
        ...
        #zhoumin
        # Create DataFrame for truth and pred with gene names
        df_data = {
        'Genes': genes,
        'Truth': truth.mean(axis=0),  # 计算每列的平均值，即每个基因的真实值
        'Prediction': pred,           # 每个基因的预测值
        'Ctrl_Means': ctrl_means      # 每个基因的对照组平均值
        }
        df = pd.DataFrame(df_data)
        filename = f"{query.replace(' ', '_')}_perturbation_data.csv"
       ...
       # df_data = {'Truth': truth.flatten(), 'Prediction': pred}
       # df = pd.DataFrame(df_data, index=pd.MultiIndex.from_product([genes, range(len(truth))], names=['Gene', 'Sample']))
        #df.reset_index(inplace=True)

        # Save DataFrame to CSV
        #csv_filename = f"{query.replace(' ', '_')}_plot_data.csv"
    ...
    def save_perturbation_data_truth(self, query):
        adata = self.adata
        gene2idx = self.node_map
        cond2name = dict(adata.obs[['condition', 'condition_name']].values)
        gene_raw2id = dict(zip(adata.var.index.values, adata.var.gene_name.values))
        ...
        query_ = [q for q in query.split('+') if q != 'ctrl']
        pred_results = self.predict([query_])
        if isinstance(pred_results, tuple):
            pred_results = pred_results[0]
        pred = pred_results['_'.join(query_)][de_idx]

        # 将pred减去'ctrl'条件下的平均值
        ctrl_means = adata[adata.obs['condition'] == 'ctrl'].to_df().mean()[de_idx].values
        pred = pred - ctrl_means

    # 创建一个DataFrame，每一行对应一个样本，每一列对应一个基因
        sample_ids = adata[adata.obs.condition == query].obs_names.tolist()
        df_data = []
        for sample_id, sample_truth in zip(sample_ids, truth):
            for gene, value in zip(genes, sample_truth):
                df_data.append({
                    'Sample_ID': sample_id,
                    'Genes': gene,
                    'Truth': value,
    ...
    # 定义文件名
        filename = f"{query.replace(' ', '_')}_truth.csv"
    ...
    # 保存DataFrame到CSV文件
        ...
        return df
    def save_perturbation_truth_delta(self, query):
        adata = self.adata
        gene2idx = self.node_map
        cond2name = dict(adata.obs[['condition', 'condition_name']].values)
        gene_raw2id = dict(zip(adata.var.index.values, adata.var.gene_name.values))
        ...
        # 将pred减去'ctrl'条件下的平均值
        ctrl_means = adata[adata.obs['condition'] == 'ctrl'].to_df().mean()[de_idx].values
        adjusted_truth = truth - ctrl_means
        sample_ids = adata[adata.obs.condition == query].obs_names.tolist()
        df_data = []
        for sample_id, sample_truth in zip(sample_ids, adjusted_truth):
            for gene, value in zip(genes, sample_truth):
                df_data.append({
                     'Sample_ID': sample_id,
                     'Genes': gene,
                     'Adjusted_Truth': value,
                }) 

        df = pd.DataFrame(df_data)

    # 定义文件名
        filename = f"{query.replace(' ', '_')}_truth_delta.csv"
        ...
        if self.split == 'simulation':
            ...
            for name, pert_list in subgroup['test_subgroup'].items():
                for pert in pert_list:
                    ...
                    for m in metrics_non_dropout:
                        if m in out_non_dropout[pert]:
                            subgroup_analysis[name][m].append(out_non_dropout[pert][m])
                        else:
                            print(f"Warning: '{m}' key not found for perturbation {pert}. Skipping.")
                       # subgroup_analysis[name][m].append(out_non_dropout[pert][m]) old zhoumin
